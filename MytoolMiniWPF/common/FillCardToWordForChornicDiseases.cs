...
{
    internal class FillCardToWordForChornicDiseases
    {
        ...
        public void ChangeStartBtnStyle(System.Windows.Controls.Button button,bool isStart)
        {
            ...
            App.Current.Dispatcher.Invoke((Action)delegate ()
            {
                ...
                MaterialDesignThemes.Wpf.ButtonProgressAssist.SetIsIndeterminate(button, isIndeterminate);
        ...
        public List<string> startProgram(string userName,  ListView listViewCOPD, ListView listViewAMI, ListView listViewApoplexy)
        {
            ...
            {
                string modPath = string.Format(@"{0}\cache\mod{1}.docx", System.IO.Path.GetDirectoryName(Process.GetCurrentProcess().MainModule.FileName), i);
                streams[i] = File.Open(modPath, FileMode.Open);
            }
            foreach (Patient person in this.patientList)
            {
            ...
            try
            {
                ...
                delegateMessageShow("报卡完毕!", $"报卡日志已自动导出\r\n" +
                $"默认保存路径:\r\n {projectDir} '报卡月份'\r\n" +
                    $"本次保存路径:{this.diagnoticsPath}");
            }
            catch (Exception ex)
            {
                delegateMessageShow($"尝试生成报卡日志出错!",$"可能的原因为未安装Microsoft Office或安装文件丢失！\r\n{ex}");
            }
            return pathList;
        }

        
        private void checkDisease(Patient person)
        {
            ...
            for (int i = 0; i < diagSselect.Count; i++)
            {
                ...
                {
                    this.patientReportedList.Add(person); //为报卡日志存储数据
                    if (i <= 4)
                    {
                    ...
                    catch (Exception ex)
                    {
                      ...
                      // delegateMessageShow("错误！",showString);
                        Toast.Show(parentWindow, $"{person.Name}没有身份证！", new ToastOptions { Icon = ToastIcons.None, ToastMargin = new Thickness(2), Time = 2000, Location = ToastLocation.OwnerCenter });
                    ...
                    this.diagnoticsPath = $"{projectDir}\\{Convert.ToDateTime(person.OutDay).ToString("yyyy年MM月")}慢病报卡日志.导出日期-{DateTime.Now.ToString("yyyy-MM-dd")}.xlsx";
        ...
        private void SaveDiagnoticsToXlsx()
        {
            ...
            string[] titles = { "报卡类别", "出院时间","姓名", "性别","年龄", "出院诊断",
                                "联系电话","身份证号","报卡者"};

            int rows = this.patientReportedList.Count;
            ...
            {
                objData[1, i] = titles[i];
            }

            // 填充数据
            for (int j = 0; j < rows; j++)
            {
                objData[j + 2, 0] = patientReportedList[j].MainDiagnose;
                objData[j + 2, 1] = patientReportedList[j].OutDay;
                objData[j + 2, 2] = patientReportedList[j].Name;
                objData[j + 2, 3] = patientReportedList[j].Gender;
                objData[j + 2, 4] = patientReportedList[j].Age;
                objData[j + 2, 5] = patientReportedList[j].OutDiagnose;
                objData[j + 2, 6] = patientReportedList[j].Phone;
                objData[j + 2, 7] = patientReportedList[j].IdCardNumber;
                objData[j + 2, 8] = patientReportedList[j].DoctorName;
            }
            // 获取数据范围并设置格式
        ...
        private void delegateMessageShow(string info,string message)
        {
            Application.Current.Dispatcher.BeginInvoke(new Action(() =>
            {
                UMessageBox.Show(info,message);
            }));
        ...
        private ListViewItem AddItem(string txt)
        {
            return new ListViewItem { Content = txt };
        }
    }
}
